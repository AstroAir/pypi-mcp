...
async def demonstrate_version_listing():
...
async def demonstrate_stats():
    ...
    async with client:
        try:
            ...
            print(f"Total PyPI size: {format_file_size(stats.total_packages_size)}")
...
async def demonstrate_dependency_analysis():
    ...
    async with client:
        try:
            ...
            print(f"Analyzing dependencies for {package_info.name} {package_info.version}")
            ...
            if package_info.requires_dist:
                ...
                if len(package_info.requires_dist) > 5:
                    print(f"  ... and {len(package_info.requires_dist) - 5} more")
            
            if package_info.provides_extra:
                print(f"Optional extras: {', '.join(package_info.provides_extra)}")
...
async def demonstrate_security_check():
    ...
    async with client:
        try:
            ...
            print(f"Security check for {package_info.name} {package_info.version}")
            
            if package_info.vulnerabilities:
                print(f"⚠️  Found {len(package_info.vulnerabilities)} vulnerabilities:")
